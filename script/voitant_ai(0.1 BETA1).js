  ...
  switch(action) {
    case 'halo':
      if (splitCommand.length > 1) {
        ...
        return 'maaf, saya tidak mengerti dengan perkataan anda!'
      }
    case 'buka':
      if (splitCommand.length > 1 && splitCommand[1] === 'aplikasi') {
        if (splitCommand.length > 2) {
          const application = splitCommand.slice(2).join(' ');
          window.open(application + '://');
        } else {
          return 'Perintah tidak lengkap. Contoh: buka web google.com';
        }
      } else {
        return 'Perintah tidak dikenali';
      }
    case 'tolong':
      if (splitCommand.length > 1 && splitCommand[1] === 'buka') {
        if (splitCommand.length > 2) {
    ...
    case 'tolong':
      if (splitCommand.length > 1 && splitCommand[1] === 'cari') {
        if (splitCommand.length > 2) {
          const search = splitCommand.slice(2).join(' ');
          window.open('https://www.google.com/search?q=' + search, '_blank');
          return 'Mencari ' + search;
        } else {
          return 'Perintah tidak lengkap. Contoh: tolong cari unggas machines';
        }
      } else {
        return 'Perintah tidak dikenali';
      }
